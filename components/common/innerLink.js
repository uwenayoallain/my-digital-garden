...
export default function InnerLink({ href, content = "Learn more", className, left = false, ...otherProps }) {
  return (
    <Link href={href}>
      {left ?
        <a className={` ${className} group m-4 block w-max rounded-full p-2 pl-10 border-4 font-medium  bg-transparent hover:ring-4 hover:ring-yellow-400 ring-0 transition  hover:text-yellow-400 dark:hover:text-indigo-500 dark:hover:ring-indigo-500  border-transparent bg-clip-padding`} {...otherProps}>
          {
