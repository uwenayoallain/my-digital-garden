...
export default function InnerLink({ href, content = "Learn more", className, left = false, ...otherProps }) {
  return (
    <Link href={href}>
      {left ?
        <a className={` ${className} group m-4 block w-max rounded-full p-2 pl-10 border-4 font-medium  bg-transparent hover:ring-4 hover:ring-gray-900 ring-0 transition hover:text-skin-base dark:hover:text-skin-base dark:hover:ring-white  border-transparent bg-clip-padding`} {...otherProps}>
          {
            <svg xmlns="http://www.w3.org/2000/svg" className="inline-block group-focus:animate-bounce group-hover:-translate-x-9  w-6 h-6 rotate-180 -translate-x-3 transition" viewBox="0 0 24 24" stroke="currentColor">
        ...
        :
        <a className={` ${className} group mt-16 m-6 block w-max rounded-full p-6 pr-12 border-4 font-medium dark:text-black text-gray-100 dark:bg-white hover:ring-4 hover:ring-gray-900 ring-0 transition hover:text-skin-base dark:hover:text-skin-base dark:hover:ring-white bg-gray-900 border-transparent bg-clip-padding`} {...otherProps}>
          {content}
          {
