...
export default function Custom404() {
    ...
    const drawApple = ctx => {
        ...
        ctx.strokeStyle = 'transparent' // '#187741' // '#8C6D1F
    ...
    return (
        <>
            <hr />
            <h1 className='text-center text-6xl font-sans font-extrabold my-4'>
                <span className=''>4<span className='text-skin-base'>0</span>4</span><span className='font-serif font-medium text-4xl '>| Page Not Found</span>
            </h1>
            <hr />
            <main className='relative '>
                <canvas
                    className='shadow-xl m-auto'
                    ref={canvasRef}
                    width={canvasWidth + 1}
                    height={canvasHeight + 1}
                />
                <div id='overlay' className='rounded-md inset-0 bg-skin-base absolute flex flex-wrap justify-center items-center'>
                    <button className='static' onClick={() => { document.querySelector('#overlay').classList.replace('flex', 'hidden') && startGame() }} >play</button>
                </div>

                <section className='w-4/6 m-auto bg-white'>
                    {
                        score > 0 && <p className='static'>Score: {score}</p>
                    }
                    <p className='static'>Highscore: {highscore > score ? highscore : score}</p>
                    {(!isLost && countDown > 0) ?
                        <button className='static !bg-green-500 hover:!bg-green-500/95' onClick={startGame}>{countDown === 4 ? 'Start Game' : countDown}</button> : ""
                    }
                </section>
                {isLost &&
                    <div className='rounded-md inset-0 bg-skin-base absolute flex flex-wrap justify-center items-center'>
                        <p className='static'>
                            {newHighscore ? `🎉New Highscore🎉` : `You scored: ${score}`}
                        </p>
                        {(!running && isLost) && <button onClick={startGame} className='static !bg-yellow-500'>{countDown === 4 ? 'Restart Game' : countDown}</button>}
                    </div>
                }
            </main>
        </>
